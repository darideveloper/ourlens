...
- **AND** SHALL NOT display "Analyse video" button during the recording
...
- **AND** SHALL display "Analyse video" button with gradient background after recording completes (camera status returns to `ready`)
...
- **AND** SHALL display "Analyse video" button with gradient background after capture completes (camera status returns to `ready`)
...
- **WHEN** the user submits frames for analysis (taps "Analyse video")
